  ...
  <div class="min-h-screen bg-slate-50 flex flex-col justify-between font-sans text-slate-800">
      ...
      <div class="max-w-4xl mx-auto flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center">
            <q-icon name="qr_code_scanner" size="24px" class="text-amber-400" />
          </div>
          <div>
    ...
    <main class="flex-1 max-w-xl w-full mx-auto p-4 sm:p-6 flex flex-col justify-center my-6 space-y-6">
      ...
      <div v-if="loadingEvent" class="text-center py-12 space-y-4">
        ...
        <p class="text-sm text-slate-500 font-medium">Carregando informações do evento...</p>
      </div>
      ...
      >
        ...
        <h2 class="text-lg font-bold text-slate-800">Evento não encontrado</h2>
      ...
      <template v-else>
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-7 shadow-sm space-y-4">
          <div class="flex items-center justify-between gap-2 flex-wrap">
            <span class="px-3 py-1 bg-blue-50 text-[#0F4C81] border border-blue-100 rounded-full text-xs font-bold uppercase tracking-wider">
              {{ event.categoria || 'Palestra / Evento' }}
            </span>
            <span class="text-xs font-semibold text-slate-500 flex items-center gap-1">
              <q-icon name="pin_drop" size="14px" class="text-slate-400" />
              {{ event.local || 'Auditório / Presencial' }}
            </span>
          </div>
          ...
          <div class="space-y-1">
            ...
            <p v-if="event.secretaria" class="text-xs font-semibold text-slate-500">
          ...
          <div class="pt-3 border-t border-slate-100 grid grid-cols-2 gap-3 text-xs">
            <div class="flex items-center gap-2 text-slate-600 bg-slate-50 p-2.5 rounded-xl">
              ...
              <div>
                ...
                <div class="font-bold text-slate-800">{{ formatarDataHora(event.dataInicio) }}</div>
              </div>
            </div>
            <div class="flex items-center gap-2 text-slate-600 bg-slate-50 p-2.5 rounded-xl">
              <q-icon name="schedule" size="18px" class="text-[#0F4C81]" />
              <div>
                <div class="text-[10px] font-bold text-slate-400 uppercase">Término</div>
                <div class="font-bold text-slate-800">{{ formatarDataHora(event.dataFim) }}</div>
              </div>
            </div>
          </div>
        </div>
