...
### Changed
- **Formulário de Criação e Edição de Cursos (`FormularioCursoDialog.vue`):**
  - Removido o campo manual de seleção de categoria.
  - A categoria do curso agora é herdada dinamicamente a partir do Eixo Temático da Trilha de Aprendizagem selecionada, ou definida automaticamente como `"Geral"` para cursos avulsos.
...
- **Validador de Certificados (`ValidarCertificado.vue` & `certificateStore.js`):**
  ...
  - Download direto do arquivo PDF oficial a partir da tela de validação.
